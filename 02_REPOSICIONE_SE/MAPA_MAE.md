# MAPA-MÃE — LIVRO 2 — REPOSICIONE-SE

**Estado vivo após ETAPA 03 — corpo principal escrito**  
**Data:** 10/09/2026  
**Snapshots históricos preservados em:** `ETAPAS/`

## Identidade da obra
**Título:** Reposicione-se  
**Linha de força:** *O mundo te ensinou a reagir. Ninguém te ensinou a existir.*

**Pergunta central:** **Agora que consigo enxergar, como mudo a posição que ocupo na minha vida?**

## Premissa
Toda pessoa já está posicionada — por escolha, repetição, proteção, influência, imposição, omissão, adiamento ou decisão deliberada. Reposicionar-se não é fabricar nova personalidade. É observar Frutos, localizar a posição atual, investigar o que a sustenta, passar pelo Filtro e praticar outra direção quando os resultados são incompatíveis com a vida desejada — dentro da margem real de segurança, recurso e responsabilidade.

## Regra de fronteira
- Pode usar episódios breves de `Morte em Vida` apenas quando ensinam uma função nova.
- Não reconta a biografia.
- Entrega integralmente o método de reposicionamento.
- Fuga Identitária aparece forte como pergunta e semente; sua anatomia completa pertence ao Livro 3.
- Nenhum livro rouba a resposta do próximo.

---

# HIERARQUIA DO LIVRO

## 1. Método central — Árvore do Discernimento™ / Método Posicione-se™
A Árvore organiza investigação e ação.

### Gramática vigente
- **Semente:** atitude, decisão ou prática plantada.
- **Solo:** ambiente e condições do cultivo.
- **Raízes:** modus operandi/mindset, crenças operantes, valores, hábitos, lealdades e lógicas que ainda sustentam o padrão.
- **Tronco:** valores, identidade praticada, limites e sustentação sob custo.
- **Galhos:** áreas concretas da vida.
- **Frutos:** resultados observáveis.
- **Pragas:** mecanismos que drenam, distorcem ou sabotam; nunca pessoas.
- **Mirante:** posição metacognitiva de observação.
- **Filtro:** critérios de realidade, lógica, ética, vínculo, tempo, autoria, segurança e proporcionalidade.
- **Poda:** interrupção responsável e específica.
- **Nova Semente:** nova atitude pequena, executável, observável e revisável.
- **Descida:** retorno à vida com ação proporcional.

### Ordem simbólica do cultivo
`Semente → Solo → Raízes → Tronco → Galhos → Frutos`

A ordem organiza leitura; não afirma causa única nem linearidade psicológica rígida.

### Ordem da investigação
`Fruto → Galho → posição atual → Tronco → Raízes → Solo/Sementes → Pragas → Mirante/Filtro → custos/recursos → Poda → Nova Semente → ação → revisão pelos Frutos`

## 2. Ambiente narrativo — A Jaula Estava Aberta
A Jaula não é segundo método. É a casa simbólica da permanência e da restrição.

`Jaula/Casa → Sofá Quente da Mentira → Sono da Negligência → Pragas/Pertencimento/Influências → Porta Possível → Soleira → Travessia → Sustentação`

### Comando transversal
**A JAULA ESTÁ ABERTA** = há evidência suficiente para parar de fingir que não viu; agora é preciso investigar o que mantém a permanência e qual movimento é realmente possível.

**Trava:** porta percebida não significa saída segura ou imediatamente disponível.

## 3. Contrato intelectual
**Deixe seu julgamento na Árvore → Suba → observe → filtre → desça.**

Se o leitor continuar discordando depois do exame, pode recolher o julgamento, confrontar a autora ou seguir outro caminho. O livro pede discernimento, não obediência.

## 4. Lei-mãe do Posicionamento
Toda pessoa já ocupa posições. Frutos ajudam a revelar a posição operante. Quando os Frutos são incompatíveis com a vida desejada, o método investiga o cultivo e busca uma posição deliberada e praticável.

## 5. 14 Leis operacionais
As Leis nascem onde o problema do leitor exige cada uma. A lista completa aparece apenas depois da experiência, no Cap. 28.

Arquivo canônico: `LEIS_DO_POSICIONAMENTO.md`.

## 6. Lentes bíblicas
- **Pelos frutos se conhece a árvore** — observação sem sentença total.
- **Parábola do Semeador** — mesma mensagem em solos diferentes; o livro explicita a adaptação autoral de Semente no método.
- **Conhecereis a verdade e a verdade vos libertará** — contato com a verdade + resposta consciente; não promessa mágica.

## 7. Sete Frutos do Posicionamento
Lentes pedagógicas de colheita consolidadas no Cap. 28:
- Verdade;
- Responsabilidade;
- Discernimento;
- Coerência;
- Coragem;
- Sabedoria;
- Legado.

**Trava:** não são estágios obrigatórios, diagnóstico, promessa de resultado ou certificado de superioridade.

---

# PATRIMÔNIO NARRATIVO OBRIGATÓRIO

## Letreiro de Neon
Valor comunicado, permissões, limites, acessos e posição. Não transfere responsabilidade por violência ao alvo dela.

## Cajueiro de Pirangi
Sol já amava árvores. O Cajueiro dá escala e estrutura à percepção e funda `Suba na Árvore`. Retorna no Epílogo.

## Casa/Jaula + Sofá Quente da Mentira
Acompanham o livro como ambiente narrativo; a arquitetura completa amadurece na Parte V.

## Ponte com Morte em Vida
Apenas o necessário para mostrar: o Livro 1 fez a autópsia; o Livro 2 pergunta o que fazer com o que agora é visível.

---

# ARQUITETURA CANÔNICA EXECUTADA — 28 CAPÍTULOS

## PRÉ-LIVRO — VOCÊ JÁ ESTÁ POSICIONADO
Contrato intelectual; tese; Letreiro; Semeador; paixão por árvores; Cajueiro; primeira subida; semente da Casa/Jaula; Fotografia de Partida.

## PARTE I — O DESPERTAR
1. **A Vitrine da Vida**  
2. **O Fruto Não Mente**  
3. **Você Já Está Posicionado**  
4. **O Automático e a Primeira Subida**

## PARTE II — SEMENTE, SOLO E RAÍZES
5. **A Mesma Semente, Solos Diferentes**  
6. **O Solo que Te Recebeu**  
7. **Raízes: Seu Modo Operante**  
8. **Mapas Herdados, Pertencimento e Merecimento**

## PARTE III — O TRONCO
9. **O Tronco: O que Você Sustenta Quando Custa**  
10. **Soberania Interna e Corpo-Sismógrafo**  
11. **O Limite Sagrado**  
12. **Acordos Conscientes**

## PARTE IV — OS GALHOS
13. **O Mapa dos Galhos**  
14. **Amor, Família e Vínculos**  
15. **Trabalho, Dinheiro, Carreira e Marca Pessoal**  
16. **Fé, Corpo, Redes, Vida Pública e Política**

## PARTE V — PRAGAS, INFLUÊNCIAS E JAULAS
17. **As Pragas da Árvore: Autopiedade não é Autocompaixão**  
18. **O Sono da Negligência**  
19. **O Sofá Quente da Mentira**  
20. **Quem Está Pensando por Você?**  
21. **A Jaula Está Aberta**

## PARTE VI — FILTRO, EVIDÊNCIAS E ESPELHOS
22. **Metacognição e Filtro da Sensatez**  
23. **Cadeia de Resultados, Evidências e Custos**  
24. **Os 14 Espelhos do Posicionamento**

## PARTE VII — PODA, NOVA SEMENTE E FRUTOS
25. **Responsabilidade, Dever e Visão de Futuro**  
26. **Poda, Limites e Luto**  
27. **Nova Semente, Travessia e Sustentação**  
28. **Novos Frutos: Desça da Árvore e Vá Viver**

## EPÍLOGO — O CAJUEIRO AINDA ESTÁ LÁ
Fechamento literário: a árvore permanece; o ponto de observação mudou; o leitor desce para viver. **A escrever e costurar na ETAPA 04.**

A função completa de cada capítulo está em `MATRIZ_CAPITULOS_FUNCOES.md`.

---

# FLUXO OFICIAL REFINADO

A ETAPA 00 registrou o fluxo inicial. Após auditoria, ele foi preservado como lógica de transformação e executado editorialmente nesta ordem:

`desconforto → observar Fruto → reconhecer posição → perceber automático → investigar Semente/Solo/Raízes → fortalecer Tronco → comparar Galhos → reconhecer Pragas/Sono/Sofá/Influências/Jaula → subir ao Mirante → passar pelo Filtro → mapear cadeia/evidências/custos → assumir responsabilidade proporcional → revisar acordos/limites → Podar → plantar Nova Semente → atravessar → sustentar → observar novos Frutos → descer e viver`

**Justificativa da organização:** a Lei-mãe e o Fruto aparecem cedo; a Jaula funciona como ambiente transversal; o leitor só filtra depois de possuir material observável suficiente; Poda ocorre apenas após verificação e responsabilidade distribuída.

---

# GALHOS DE APLICAÇÃO

O mesmo método é demonstrado em:
- relacionamentos e família;
- trabalho, dinheiro e carreira;
- marca pessoal e presença;
- fé e vida interior;
- corpo/sexualidade quando pertinente;
- redes, influência e vida pública;
- política como área de discernimento, não propaganda.

A mesma pessoa pode sustentar posições diferentes em Galhos diferentes.

---

# FERRAMENTAS E WORKBOOK

No livro: microaplicações e ferramentas essenciais.  
No Workbook: testes completos, fichas, pontuações e acompanhamento.

Rota detalhada: `MAPA_EXERCICIOS_TESTES.md`.

Instrumentos principais:
- Fotografia de Partida;
- Subida de 90 segundos;
- Teste da Árvore;
- Corpo-Sismógrafo;
- Limite Responsável;
- Acordo Consciente;
- Mapa dos Galhos;
- Autopiedade × Autocompaixão;
- Custo do Sofá;
- Filtrar Influências / Autonomia;
- Jaula / Porta Possível;
- Filtro da Sensatez;
- Cadeia de Resultados / Custos;
- 14 Espelhos;
- Responsabilidade Distribuída;
- Plano de Poda Responsável;
- Nova Semente;
- Checkpoints de sustentação;
- Retorno à Fotografia de Partida.

---

# PRIMEIROS 10% KINDLE

Arquivo específico: `MAPA_10_PORCENTO_KINDLE.md`.

Antes do corte, o leitor precisa:
1. conhecer a voz de Sol;
2. poder discordar;
3. entender que já está posicionado;
4. encontrar Letreiro, Semeador e Cajueiro;
5. usar a Árvore ao menos uma vez;
6. entrar efetivamente na Parte I;
7. ficar com pergunta em movimento.

A calibração exata do corte será revista na ETAPA 04/08 com o tamanho integral do manuscrito.

---

# RIGOR, SEGURANÇA E FONTES

- posição ≠ consentimento;
- responsabilidade ≠ culpa retrospectiva;
- violência continua pertencendo a quem a pratica;
- Fruto isolado ≠ identidade/causa completa;
- pessoa ≠ Praga/Espelho;
- corpo sinaliza; não sentencia;
- influência ≠ manipulação automática;
- porta visível ≠ capacidade imediata de saída;
- Poda ≠ ruptura compulsória;
- desconforto ≠ prova de crescimento;
- recaída ≠ identidade/retorno automático ao zero;
- Sete Frutos ≠ escala clínica ou moral;
- em risco/coerção/dependência, segurança e apoio precedem confronto;
- ciência só entra com amplitude compatível com a fonte.

Registro vivo: `REGISTRO_DE_ALEGACOES_E_FONTES.md`.

---

# PONTE PARA O LIVRO 3

Pergunta transversal permitida: **essa posição tem minha assinatura?**

Pergunta de saída do corpo principal: **quem é o Eu que está escolhendo essa nova posição?**

O Livro 2 não desenvolve integralmente apagamento identitário, identidades de sobrevivência, terceirização do Eu ou reconstrução identitária profunda.

---

# ARQUIVOS QUE GOVERNAM A PRODUÇÃO

1. `BIBLIA_EDITORIAL_REPOSICIONE_SE.md`
2. `MAPA_MAE.md`
3. `DICIONARIO_CANONICO_DO_METODO.md`
4. `LEIS_DO_POSICIONAMENTO.md`
5. `MATRIZ_CAPITULOS_FUNCOES.md`
6. `MICROARQUITETURA_28_CAPITULOS.md`
7. `MAPA_10_PORCENTO_KINDLE.md`
8. `MAPA_STORYTELLINGS.md`
9. `MAPA_EXERCICIOS_TESTES.md`
10. `MAPA_IMAGENS_ILUSTRACOES.md`
11. `REGISTRO_DE_ALEGACOES_E_FONTES.md`
12. `PENDENCIAS_AUTORA.md`
13. `PROTOCOLO_CONTINUIDADE_ESCRITA_EM_MASSA.md`
14. `MANUSCRITO/`.

## Estado
**ETAPA 03 concluída: Pré-livro + 28 capítulos escritos. ETAPA 04 ativa: Epílogo `O Cajueiro Ainda Está Lá` + costura integral do livro.**
