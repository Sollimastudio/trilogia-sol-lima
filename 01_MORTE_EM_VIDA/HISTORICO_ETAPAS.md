# HISTÓRICO DE ETAPAS — MORTE EM VIDA

Este arquivo registra a evolução estrutural e textual do Livro 1 sem apagar estados anteriores.

## ETAPA 00 — MAPA-MÃE MACRO
**Data:** 10/09/2026  
**Status:** ✓ registrada.

- pergunta central: Como eu desapareci de mim?;
- 12 blocos macro;
- moldura forense;
- clímax no Sepultamento Simbólico;
- ponte para Reposicione-se.

---

## ETAPA 01 — ENGENHARIA CAPÍTULO A CAPÍTULO
**Data:** 10/09/2026  
**Status:** ✓ arquitetura operacional inicial concluída.

Previsão inicial de 45 capítulos + interlúdios + notas forenses mantida como referência flexível.

Snapshot: `ETAPAS/01_ENGENHARIA_CAPITULOS_2026-09-10.md`

---

## ETAPA 02 — CONTRATO EMOCIONAL + ORIGEM
**Data:** 10/09/2026  
**Status:** ✓ primeira versão concluída.

Primeira escrita: MV-00 a MV-03.  
Snapshots 02A, 02B e fechamento do Lote 00+01 preservados.

---

## ETAPA 03 — CORPO, SILÊNCIO E IMAGINAÇÃO
**Data:** 10/09/2026  
**Status:** ✓ primeira versão concluída.

Primeira escrita: MV-04 a MV-07.  
Mangas preservado como reserva.

Snapshot: `ETAPAS/03_LOTE02_CORPO_SILENCIO_IMAGINACAO_2026-09-10.md`

---

## ETAPA 04 — COMPETÊNCIA, PAI E RISCO + AUDITORIA DA PARTE I
**Data:** 10/09/2026  
**Status:** ✓ primeira escrita integrada.

Primeira escrita: MV-08, MV-09, MV-10 e MV-11.  
Cirurgia: MV-03A, MV-03B e MV-I01 reintegrados.  
Reino das Galinhas preservado em `RESERVAS/`.

Snapshot: `ETAPAS/04_LOTE03_E_PARTEI_INTEGRADA_2026-09-10.md`

Estado acumulado: 15 unidades V1.

---

## ETAPA 05 — ENTRADA NOS PAPÉIS ADULTOS
**Data:** 10/09/2026  
**Status:** ✓ primeira escrita + revisão de lote.

A ordem conceitual antiga foi corrigida pela cronologia. Maternidade foi retirada do lote para não comprimir 1994–1997.

Primeira escrita: MV-12, MV-13, MV-14, MV-I04, MV-15 e MV-16.

Snapshot: `ETAPAS/05_LOTE04_ENTRADA_PAPEIS_ADULTOS_2026-09-10.md`

Estado acumulado: 21 unidades V1.

---

## ETAPA 06 — VIDA CONJUGAL, MATERNIDADE, COMPETÊNCIA, RISCO E RUPTURA
**Data:** 10/09/2026  
**Status:** ✓ primeira escrita integrada da Parte II.

26 núcleos receberam destino explícito antes da prosa.

Primeira escrita: MV-17 a MV-22.

Snapshot: `ETAPAS/06_LOTE05_PARTEII_INTEGRADA_2026-09-10.md`

Estado acumulado: 27 unidades V1.

---

## ETAPA 07 — PARTE III · MOVIMENTO A: A FALSA RESSURREIÇÃO
**Data:** 10/09/2026  
**Status:** ✓ concluída como primeira escrita + revisão de arco.

Primeira escrita: MV-I05 e MV-23 a MV-27.

O pós-2010 foi dividido em dois mecanismos: violência corporal/doméstica do Chileno e cerco materno posterior. O Portão foi preservado para a etapa seguinte.

Snapshot: `ETAPAS/07_PARTEIII_MOVIMENTO_A_FALSA_RESSURREICAO_2026-09-10.md`

Estado acumulado: 33 unidades V1.

---

## ETAPA 08 — PARTE III · MOVIMENTO B: O CERCO CONTRA A MÃE
**Data:** 10/09/2026  
**Status:** ✓ concluída como primeira escrita + auditoria integrada.

Matriz E1–E6 criada. Núcleos processuais permaneceram predominantemente E3 porque documentos primários não foram auditados nesta etapa.

Primeira escrita: MV-28 a MV-31.

Snapshot: `ETAPAS/08_CERCO_CONTRA_A_MAE_2026-09-10.md`

Estado acumulado: 37 unidades V1.

---

## ETAPA 09 — PARTE III · MOVIMENTO C: A VIDA QUE CONTINUOU
**Data:** 10/09/2026  
**Status:** ✓ concluída como primeira escrita + revisão + auditoria integrada da Parte III.

Primeira escrita: MV-32 a MV-35.

`A Cadáver que Tomava Café` tornou-se capítulo-eixo. `O Amor sem Toque`, `O Purgatório` e núcleos tardios foram reservados conforme fronteiras editoriais.

Snapshot: `ETAPAS/09_CADAVER_FUNCIONAL_PARTEIII_INTEGRADA_2026-09-10.md`

Estado acumulado: 41 unidades V1.

---

## ETAPA 10 — PARTE IV · ABERTURA DA AUTÓPSIA
**Data:** 10/09/2026  
**Status:** ✓ primeira escrita + auditoria de arquitetura.

O antigo Ato VI foi auditado em 6 lotes / 72 núcleos.

Primeira escrita:
- MV-36 — A Pergunta no Ônibus;
- MV-37 — A Verdade que Deixou de Ser Versículo.

Decisões: responsabilidade separada de culpa; OSV separado de Sistema Operacional da Vida; senhor do café não fundido a Paulo Bento; ciência e método preservados para etapas próprias.

Snapshot: `ETAPAS/10_AUTOPSIA_ABERTURA_VERDADE_RESPONSABILIDADE_2026-09-10.md`

Estado acumulado: 43 unidades V1.

---

## ETAPA 11 — MATEMÁTICA DO PERDÃO + SEPULTAMENTO SIMBÓLICO
**Data:** 10/09/2026  
**Status:** ✓ primeira escrita + revisão do clímax.

VI-L3-001–012 foi auditado antes da prosa.

Primeira escrita:
- MV-38 — A Matemática do Perdão — V1;
- MV-39 — O Sepultamento Simbólico — V1.1.

Decisões centrais:
- perdão como experiência autoral, não obrigação universal;
- 490 como formulação autoral, não terapia;
- reconciliação/acesso separados de perdão;
- Oripe não é causa clínica;
- Sepultamento é da dívida percebida, não da pessoa;
- `Eu desocupei o caixão` preservado como frase única de clímax;
- pós-clímax reduzido a silêncio, porta, corredor e vida continuando;
- antiga lâmina técnica pós-clímax retirada dessa posição.

Snapshot: `ETAPAS/11_PERDAO_SEPULTAMENTO_CLIMAX_2026-09-10.md`

Estado acumulado: 45 unidades V1/V1.1.

---

## ETAPA 12 — A INVESTIGADORA GANHA LINGUAGEM
**Data:** 10/09/2026  
**Status:** ✓ concluída como primeira escrita + auditoria + revisão.

VI-L4-001–012 foi auditado antes da prosa e mantido em um capítulo para evitar currículo/repetição.

Primeira escrita:
- MV-40 — A Investigadora Ganha Linguagem — V1.

Centro:
`EU VI / EU OUVI / EU LI / ALGUÉM ME CONTOU / EU CONCLUÍ DEPOIS`.

Marcos documentais usados com contenção:
- DISC Advanced — 17/11/2021 — 50h;
- Master Love Professional — 19/11/2021 — 140h.

Ciência não substituiu fé; diagnóstico não virou identidade; CSI da própria mente nasceu da prática de distinguir evidências e não como slogan.

Snapshot: `ETAPAS/12_INVESTIGADORA_GANHA_LINGUAGEM_2026-09-10.md`.

Estado acumulado: 46 unidades V1/V1.1.

---

## ETAPA 13 — A MULHER QUE QUASE NÃO VIVEU + RECUPERAÇÃO DE VOZES
**Data:** 10/09/2026  
**Status:** ✓ concluída como primeira escrita + cirurgia retroativa controlada.

### Auditoria de VI-L5
Arquivo: `LOTES/AUDITORIA_ETAPA13_MULHER_E_VOZES_RECUPERADAS.md`.

Foram preservadas as lacunas:
- data do Mulher Magnética;
- curso/instituição/título/data do TCC;
- ordem fina entre Mulher Magnética, DISC, Master Love e TCC;
- data do nome Magnetus;
- nascimento formal do Relacione-se.

### Achado de continuidade
A busca no acervo mostrou que três peças estruturais antigas não haviam sido transplantadas para o novo repositório capítulo a capítulo.

#### 1. MV-P01 — Prólogo — O Portão — ● V1
Reintegrado entre a Nota Forense e 1978.

Função: cold open sensorial, sem explicação processual. O prólogo mostra o efeito; MV-28/MV-29 mantêm o desenvolvimento cronológico do Cerco.

#### 2. MV-M01 — Agora Eu Não Posso Errar — ● V1 reintegrada
Monólogo de aproximadamente 3 mil palavras recuperado do acervo e transplantado integralmente.

Posição canônica:
`MV-22 → MV-M01 → MV-I05`.

Função: preservar a consciência real do pós-divórcio — medo de pecado, culpa, corpo julgado, filhos/teto e expectativa de direção divina — sem implantar autoestima, posicionamento ou linguagem futura na personagem.

#### 3. MV-I06 — A Criança que Não Devia Nada — ● V1
Posição canônica:
`MV-39 → MV-I06 → MV-40`.

Função: retirar a criança do banco dos réus depois do Sepultamento e antes da intelectualização da experiência.

A ingenuidade foi definida como falta de conhecimento posterior, nunca falta de inteligência.

### Mapa especial criado
`MAPA_VOZES_E_MOLDURAS.md` passa a proteger Prólogo, Monólogos e Interlúdios contra cortes silenciosos.

### Capítulo principal da etapa
#### MV-41 — A Mulher que Quase Não Viveu — ● V1
A mulher passa de objeto de estudo para sujeito de experiência.

Curva:
`função → preferência → corpo sem tribunal → Mulher Magnética como experiência formativa → desejo sem direção automática → limite → presença → TCC com lacunas preservadas → experiência observada → Magnetus como organização posterior → autorrelacionamento → Relacione-se como consequência`.

Mulher Magnética é atribuído a Vanessa de Oliveira no nível canônico atual. TCC permanece sem moldura acadêmica inventada. Magnetus e Relacione-se não recebem linguagem de venda.

### Fecho
`Eu já não precisava que alguém me dissesse quem eu seria.`

### Revisão
`LOTES/REVISAO_ETAPA13_MULHER_E_VOZES_RECUPERADAS.md`.

### Snapshot
`ETAPAS/13_MULHER_QUASE_NAO_VIVEU_E_VOZES_RECUPERADAS_2026-09-10.md`.

### Checklist
`ACOMPANHAMENTO_ETAPA_13.md`.

### Estado acumulado
**50 unidades possuem primeira escrita V1/V1.1.**

### Próxima etapa
**ETAPA 14 — Autoria, Memória, IA e Legado.**

Comando: `PROMPTS/ETAPA_14_COMANDO_ATUAL.md`.

---

## Regra de atualização
Novas etapas são acrescentadas ao final. Não apagar etapas anteriores para fazer o histórico parecer linear ou perfeito.

---

## REVISÃO 13R — LEITURA GLOBAL ANTES DE CONTINUAR
**Data:** 10/09/2026.  
**Pedido:** “Revise tudo para continuar”.  
**Status:** leitura de desenvolvimento das 50 unidades concluída; quatro revisões localizadas de prosa e orientação operacional gravadas.

### Alcance
Leitura do manuscrito existente, mapas e arquivos de continuidade, com confronto de correções autorais recuperadas da Biblioteca. Não equivale a revisão Beta integral, verificação independente de toda a biografia, montagem Alfa, diagramação ou aprovação para publicação.

### Prosa revisada
- MV-04 — V1.1: pensamento infantil direto e retirada de “teimosia” para interpretar medo.
- MV-06 — V1.1: fantasia infantil de ser anjo, reflexão posterior separada e consulta do balanço preservada fora dos bastidores da prosa.
- MV-I04 — V1.1: prazer de cantar com Elza coexistindo com obrigação de testemunho, conforme correção autoral.
- MV-22 — V1.1: orientação religiosa corrigida para princípios bíblicos, sem nomes/números incertos; compreensão atual distinguida da culpa vivida em 2010.

As formulações novas são redação literária proposta, não transcrição literal da mente nem aprovação autoral definitiva.

### Organização
Prólogo reincluído no mapa da Parte I; mapas das Partes I/II e vozes atualizados; roteiro antigo sincronizado para impedir conflito entre números históricos e etapas efetivas. ETAPA 14 continua sendo Autoria, Memória, IA e Legado. Camadas finais recebem códigos F01–F07 e não são consideradas executadas pelos números antigos.

### Pendências principais
Excesso de explicação/bastidores, repetições, antecipação de descobertas, ritmo fragmentado, densidade de acontecimentos conjugais, integração de queda do tanque/batismo e consulta do balanço, reservas da infância, datas e documentos pendentes, escrita do encerramento e futura Beta.

O comando da ETAPA 14 passou a exigir cuidado com a interpretação de Mãe-Véia, divergência 22/25 de dezembro de 2025, distinção Sepultamento aos 39/diagnóstico referido aos 43 e relato mais recente de manejo dos tremores. Nenhuma data conflitante foi decidida por suposição.

### Registros
- `LOTES/REVISAO_GLOBAL_POS_ETAPA13.md` — inventário comentado das 50 unidades e prioridades;
- `ETAPAS/13R_BASE_PRE_REVISAO_2026-09-10.md` — referências anteriores;
- `ETAPAS/13R_REVISAO_GLOBAL_2026-09-10.md` — fechamento da revisão.

### Preservação
Snapsh​ots históricos 00–13 não foram alterados. Versões anteriores dos textos permanecem no Git. Nenhuma alteração foi feita nas pastas dos Livros 2/3 ou no Universo Mestre nesta revisão.

### Saída
**50 unidades continuam existentes. ETAPA 14 preparada, não executada.** Pode continuar a produção; não declarar o livro pronto para publicar.